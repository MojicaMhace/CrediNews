...
async function handleUrlVerification() {
    ...
    try {
        const kcResp = await fetch(`https://credinews-factcheck.onrender.com/api/fact-check`, {
            method: 'POST',
...
async function handleFacebookVerification() {
    ...
    if (url && !effectiveContent) {
        try {
            const resp = await fetch(`https://credinews-factcheck.onrender.com/api/fact-check`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ url })
            });
