...
function renderItem(doc) {
  ...
  li.innerHTML = `
    ...
    <div class="content ${hasLink ? 'clickable' : ''}">
    ...
    ${!isRead ? `<button class="btn-pill" onclick="markOneRead(event, '${doc.id}')">Mark Read</button>` : ''}
  `;
    ...
    db.collection('pending_verifications')
      ...
      .onSnapshot(async snap => {
        ...
        try { localStorage.setItem(processedKey, JSON.stringify(processed)); } catch(_){}
          ...
          console.error("Pending verifications listener error:", error);
      });
