...
class AuthManager {
    ...
    init() {
        this.bindEvents();
        // this.initTheme(); // Moved to script.js to avoid redundancy
    ...
    // initTheme() removed to avoid redundancy with script.js
    ...
    // Check for redirect result (e.g. from Google Sign-in redirect)
    async checkRedirectResult() {
        try {
            const result = await firebase.auth().getRedirectResult();
            if (result.user) {
                console.log('✅ Redirect result found. Processing user...');
                const user = result.user;
                const idToken = await user.getIdToken();
                await this.processGoogleUser(user, idToken, 'Signing in');
            }
        } catch (error) {
    ...
    async handleGoogleAuth(type) {
        ...
        try {
            ...
            await this.processGoogleUser(user, idToken, actionText);

        } catch (error) {
            console.error('Google auth error:', error);
            this.showError(`${actionText} with Google failed: ${error.message}`);
        }
    }

    async processGoogleUser(user, idToken, actionText) {
        const userRef = this.db.collection('users').doc(user.uid);
    ...
    async handleGoogleSignupSubmit(e, pendingData) {
        ...
        try {
            ...
            this.showSuccess(`Welcome, ${fullName}! Registration complete. Redirecting...`);
    ...
    showLoginForm() {
        ...
        if (loginForm) {
            ...
            // Only clear history if we are managing the login form
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function checkAuthStatus() {
        ...
        if (authData) {
            try {
                // Only redirect to index.html if we are on a guest-only page (login/register)
                const path = window.location.pathname;
                const isGuestPage = path.includes('login.html') || path.includes('register.html');
                
                if (JSON.parse(authData).isAuthenticated && isGuestPage) {
