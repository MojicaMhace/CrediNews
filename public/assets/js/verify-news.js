...
// Integrated: New Localhost Logic + Old Notification Features + Bullet Point Explanations
...
// --- Integrated UI Logic (Replacing verification-shared.js) ---

document.addEventListener('DOMContentLoaded', function() {
    initializeVerifyPageTabs();
    initializeVerifyInputListeners();
});

function initializeVerifyPageTabs() {
    const fbBtn = document.getElementById('show-facebook-verify');
    const poserBtn = document.getElementById('open-poser-detection');
    
    // Ensure we are on the Verify News tab
    if (fbBtn) fbBtn.classList.add('active');
    if (poserBtn) poserBtn.classList.remove('active');

    // Redirect to Poser Detection page
    if (poserBtn) {
        poserBtn.addEventListener('click', () => {
            window.location.href = 'poser-detection.html';
        });
    }
}

function initializeVerifyInputListeners() {
    const contentInput = document.getElementById('facebook-content');
    const charCount = document.getElementById('facebook-char-count');
    ...
    if (contentInput && charCount) {
        contentInput.addEventListener('input', function() {
            ...
            // Also trigger the existing validation
            if (typeof updateVerifyButtons === 'function') {
                updateVerifyButtons();
            }
        });
    }
...
function createCleanDbPayload(result, url, contentText, platformLabel, analysisType) {
  ...
  return {
    ...
    userId: firebase.auth().currentUser ? firebase.auth().currentUser.uid : 'anonymous', // Redundant field for safety
...
async function checkExistingVerification(url, contentText) {
    try {
        ...
        if (url) {
            ...
            qs = await db.collection('facebook_verification_results').where('canonicalUrl', '==', canon).get();
            if (qs.empty) {
                qs = await db.collection('facebook_verification_results').where('url', '==', url).get();
            }
            if (!qs.empty) {
                // Sort in memory to find the best match:
                // 1. Prefer records with images (imageUrl, image_url, image, postImage)
                // 2. Then most recent
                const sortedDocs = qs.docs.sort((a, b) => {
                    const da = a.data();
                    const db = b.data();
                    
                    const hasImgA = !!(da.imageUrl || da.image_url || da.image || da.postImage);
                    const hasImgB = !!(db.imageUrl || db.image_url || db.image || db.postImage);
                    
                    if (hasImgA && !hasImgB) return -1;
                    if (!hasImgA && hasImgB) return 1;

                    const ta = da.analyzed_at ? (da.analyzed_at.seconds || 0) : 0;
                    const tb = db.analyzed_at ? (db.analyzed_at.seconds || 0) : 0;
                    return tb - ta;
                });
                const d = sortedDocs[0];
                const data = d.data();
                return { id: d.id, ...data, verificationCount: data.verificationCount || qs.size };
            }
        } else if (contentText && contentText.trim()) {
            const contentHash = hashString(contentText.trim());
            qs = await db.collection('facebook_verification_results').where('contentHash', '==', contentHash).get();
            if (qs.empty) {
                qs = await db.collection('facebook_verification_results').where('analyzedText', '==', contentText.trim()).get();
            }
            if (!qs.empty) {
                // Sort in memory to find the best match (Image > Date)
                const sortedDocs = qs.docs.sort((a, b) => {
                    const da = a.data();
                    const db = b.data();
...
async function handleUrlVerification() {
    ...
    // Try to extract key claim from URL first (Localhost Endpoint)
    try {
    ...
    try {
        ...
        if (existing) {
            // FIX: If existing verification is not mine, clone it so it appears in My Verifications
            if (window.firebase && firebase.firestore && currentUser) {
                const uid = currentUser.uid;
                // Check if the found record belongs to someone else
                if (existing.userID !== uid && existing.uid !== uid && existing.userId !== uid) {
                    try {
                        const cloneData = { ...existing };
                        delete cloneData.id; // Allow new ID generation
                        cloneData.userID = uid;
                        cloneData.userId = uid; // Add redundant field for safety
                        cloneData.userEmail = currentUser.email;
                        cloneData.analyzed_at = firebase.firestore.FieldValue.serverTimestamp();
                        cloneData.verificationCount = 1; // Reset count for this user
                        cloneData.originalId = existing.id; // Save original ID for reference/image fallback
                        
                        // Ensure image URL is carried over from any possible field
                        // If no remote image, try to point to the local trend image of the original ID
                        let bestImg = existing.imageUrl || existing.image_url || existing.image || existing.postImage || null;
                        if (!bestImg && existing.id) {
                            bestImg = 'assets/images/old_trends/' + existing.id + '.jpg';
                        }
                        cloneData.imageUrl = bestImg;
                        
                        const newDoc = await firebase.firestore().collection('facebook_verification_results').add(cloneData);
                        existing.id = newDoc.id; // Update reference to use new doc
                        existing.userID = uid;
                        // createVerificationNotification(newDoc.id);
                        console.log('Cloned existing verification to user history:', newDoc.id);
                    } catch (err) {
                        console.error('Error cloning verification for user history:', err);
                        showNotification('Note: Could not save to history (Permission Error)', 'warning');
                    }
                }
            }

            let currentCount = existing.verificationCount || 1;
            if (window.firebase && firebase.firestore) {
                try {
                    const docRef = firebase.firestore().collection('facebook_verification_results').doc(existing.id);
                    if (typeof existing.verificationCount === 'number') {
                         await docRef.update({ verificationCount: firebase.firestore.FieldValue.increment(1) });
                         currentCount = existing.verificationCount + 1;
                    } else {
                         await docRef.update({ verificationCount: 2 });
                         currentCount = 2;
                    }
                } catch (e) {
                    console.error('Error incrementing verification count:', e);
                }
            }
        ...
        if (window.firebase && firebase.firestore && currentUser) {
            try {
                const docRef = await firebase.firestore().collection('facebook_verification_results').add({
                  platform: 'Web',
                  analysis: 'web-url',
                  url: url || null,
                  canonicalUrl: normalizeUrlForCache(url || ''),
                  ...
                  userId: currentUser.uid, // Redundant field for safety
                  userEmail: currentUser.email,
                  analyzed_at: firebase.firestore.FieldValue.serverTimestamp()
                });
                createVerificationNotification(docRef.id);
            } catch (e) {
        ...
        showVerificationResult('url', {
            ...
            zylaExplanation: (result && result.zyla && typeof result.zyla.explanation === 'string') ? result.zyla.explanation : null,
            details: Array.isArray(result && result.explanations) ? result.explanations : [],
...
async function handleFacebookVerification() {
    ...
    try {
        ...
        if (existing) {
            ...
            if (window.firebase && firebase.firestore && currentUser) {
                ...
                if (existing.userID !== uid && existing.uid !== uid && existing.userId !== uid) {
                    try {
                        ...
                        delete cloneData.id;
                        cloneData.userID = uid;
                        cloneData.userId = uid; // Redundant field for safety
                        cloneData.userEmail = currentUser.email;
                        cloneData.analyzed_at = firebase.firestore.FieldValue.serverTimestamp();
                        cloneData.verificationCount = 1;
                        cloneData.originalId = existing.id; // Save original ID for reference/image fallback
                        ...
                        existing.userID = uid;
                        // createVerificationNotification(newDoc.id);
                        console.log('Cloned existing FB verification to user history:', newDoc.id);
                    } catch (err) {
                        console.error('Error cloning FB verification for user history:', err);
                        showNotification('Note: Could not save to history (Permission Error)', 'warning');
                    }
                }
            }
            ...
            if (existing.poserDetection && existing.poserDetection.raw) {
                ...
                try {
                    let poserSourceUrl = (existing.url || url || '').trim();
                    ...
                    if (poserTarget && poserTarget.includes('facebook.com')) {
                        ...
                        if (pd) {
                            cachedPoserHtml = buildPoserSummaryHtml(pd);
                            try {
            ...
            try {
                if (existing.id && poserPayload) {
                    await firebase.firestore().collection('facebook_verification_results').doc(existing.id).set({ poserDetection: poserPayload }, { merge: true });
                }
            } catch(_) {}
            // showNotification('Loaded from existing verification.', 'success');
            return;
    ...
    if (window.firebase && firebase.firestore) {
        try {
            await firebase.firestore().collection('pending_news_verification').add({
                url: url || null,
    ...
    try {
        ...
        // Ensure user fields are set correctly from the awaited user object
        if (currentUser) {
            cleanPayload.userID = currentUser.uid;
            cleanPayload.userId = currentUser.uid;
            cleanPayload.userEmail = currentUser.email;
        }
        ...
        if (window.firebase && firebase.firestore) {
            try {
                ...
                createVerificationNotification(resultId);
                // --------------------------------
...
function showFacebookVerificationResult(type, data) {
    ...
    const zylaCombined = combinedClaims.filter(c => c.source === 'zyla');
    const hasGoogleUrl = googleCombined.some(c => !!c.url);

    const reviewedClaimsPanelGoogle = (googleCombined.length > 0) ? `
        <div class="panel">
            <div class="panel-title"><span class="label">Google Fact Check</span></div>
            <ul>
                ${googleCombined.map(c => `
                    <li>
                        ...
                        ${c.url ? `<div><a href="${c.url}" target="_blank">View fact check</a></div>` : `<div><em>No fact-check URL available.</em></div>`}
                    </li>
                `).join('')}
            </ul>
        </div>
    ...
    const reviewedClaimsPanelZyla = (zylaCombined.length > 0) ? `
        <div class="panel">
            <div class="panel-title"><span class="label">Zyla Fact Check</span></div>
            <ul>
                ${zylaCombined.map(c => `
                    <li>
                        <div><strong>Claim:</strong> ${safeTextForHtml(c.claim || 'N/A')}</div>
                        <div><strong>Reviewer:</strong> ${safeTextForHtml(c.reviewer || 'Zyla Labs')}</div>
                        <div><strong>Rating:</strong> ${safeTextForHtml(c.rating || 'Unrated')}</div>
                        <div><strong>Explanation:</strong> ${safeTextForHtml((data.zylaExplanation && data.zylaExplanation.trim()) ? data.zylaExplanation : 'ZYLA did not provide an explanation for this claim')}</div>
    ...
    const factCheckDetailsSection = `
        ${realPanelHtml}
        ${fakePanelHtml}
        ${(!realPanelHtml && !fakePanelHtml) ? reviewedClaimsPanelGoogle : ''}
        ${reviewedClaimsPanelZyla}
    `;
...
function getFacebookScoreSummary(score) {
    if (score >= 75) {
        ...
        return 'Results are mixed. This may be from a trusted publisher but lacks specific fact-checks, or it contains conflicting information.';
    } else if (score >= 46) {
        return 'We could not find sufficient evidence to verify this content. Please verify the claims before sharing.';
    } else {
        return 'This content has been flagged as false, misleading, or unreliable. Please verify before sharing.';
    } 
}
...
async function showVerificationResult(type, data) {
    ...
    const zylaCombined2 = combinedClaims.filter(c => c.source === 'zyla');
    const hasGoogleUrl2 = googleCombined2.some(c => !!c.url);

    const reviewedClaimsPanelGoogle2 = (googleCombined2.length > 0) ? `
        <div class="panel">
            <div class="panel-title"><span class="label">Google Fact Check</span></div>
            <ul>
                ${googleCombined2.map(c => `
                    <li>
                        <div><strong>Claim:</strong> ${safeTextForHtml(c.claim || 'N/A')}</div>
                        <div><strong>Reviewer:</strong> ${safeTextForHtml(c.reviewer || 'Unknown reviewer')}</div>
    ...
    const reviewedClaimsPanelZyla2 = (zylaCombined2.length > 0) ? `
        <div class="panel">
            <div class="panel-title"><span class="label">Zyla Fact Check</span></div>
            <ul>
                ${zylaCombined2.map(c => `
    ...
    const factCheckDetailsSection = `
        ${realPanelHtml2}
        ${fakePanelHtml2}
        ${(!realPanelHtml2 && !fakePanelHtml2) ? reviewedClaimsPanelGoogle2 : ''}
        ${reviewedClaimsPanelZyla2}
        ${(!hasGoogleUrl2) ? `
            <div class="panel">
...
/* --- REQUEST MANUAL NEWS VERIFICATION (INTEGRATED) --- */
window.submitNewsVerificationRequest = async function(resultId, url, type, score) {
    ...
    try {
        ...
        // Always create notification if user is logged in
        if (user) {
            if (typeof window.createVerificationNotification === 'function') {
                await window.createVerificationNotification(resultId);
            }
        }
...
function getManualRequestButtonHtml(data, type) {
    ...
    return `
    <div class="manual-request-box" style="margin-top: 20px; padding: 16px; border-top: 1px solid rgba(148,163,184,0.15); text-align: center; background: rgba(15, 23, 42, 0.3); border-radius: 12px;">
        <p class="manual-request-text" style="font-size: 0.85rem; color: #94a3b8; margin: 0 0 12px 0;">
            Is this analysis incorrect? You can request a human review.
        </p>
        <button id="news-req-btn-${safeId}" 
                class="manual-request-btn news-req-btn"
