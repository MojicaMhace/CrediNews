...
function tsMillis(item) {
  ...
  // If it has reviewedAt, prioritize it
  if (item.reviewedAt) {
      return getMs(item.reviewedAt);
  }

  // Otherwise check analyzed_at
  if (item.analyzed_at) {
      return getMs(item.analyzed_at);
  }

  // Fallback: treat item itself as timestamp (legacy behavior)
  return getMs(item);
}
...
function applyFiltersAndRender(uid) {
  ...
  if (currentSort === 'date_desc') {
    docs.sort((x,y) => tsMillis(y) - tsMillis(x));
  } else if (currentSort === 'date_asc') {
    docs.sort((x,y) => tsMillis(x) - tsMillis(y));
...
function renderCards(data) {
  ...
  data.forEach(function(item){
    ...
    // Define the paths for fallbacks
    const docId = item.id || '';
    const localImg = 'assets/images/old_trends/' + docId + '.jpg'; // Assumes you save them as .jpg
    const defaultLogo = 'assets/images/logo.png';

    const imgSrc = item.imageUrl || item.image_url || item.image || item.postImage || '';
    
    // Updated imageHtml with multi-step fallback logic
    const imageHtml = imgSrc
      ? '<div class="trend-image-container">' +
          '<img src="'+safeText(imgSrc)+'" ' +
               'alt="Trend Image" loading="lazy" referrerpolicy="no-referrer" ' +
               'style="width:100%; height:180px; object-fit:cover;" ' +
               'onerror="this.onerror=null; this.src=\''+localImg+'\'; this.onerror=function(){ ' +
                  'this.src=\''+defaultLogo+'\'; this.style.width=\'100px\'; this.style.height=\'100px\'; ' +
                  'this.style.objectFit=\'contain\'; this.style.margin=\'auto\'; this.style.display=\'block\'; ' +
               '};">' +
        '</div>'
      : '<div class="trend-image-container">' +
          '<img src="'+localImg+'" style="width:100%; height:180px; object-fit:cover;" ' +
          'onerror="this.src=\''+defaultLogo+'\'; this.style.width=\'100px\'; this.style.height=\'100px\'; this.style.objectFit=\'contain\';">' +
        '</div>';
...
function getFactCheckedByHtml(data) {
  const sources = [];
  const claims = data.reviewedClaims || [];

  // 1. Zyla Labs
  const hasZylaClaims = claims.some(c => String(c.source||'').toLowerCase() === 'zyla') || 
                        (Array.isArray(data.claimAnalysis) && data.claimAnalysis.some(c => c.source === 'zyla'));
  ...
  const hasGoogle = claims.some(c => String(c.source||'').toLowerCase() === 'google') ||
                    data.hasGoogleClaims || 
  ...
  // 3. Admin
  if (data.adminReviewed === true || data.aiVerdict === 'Verified by Admin') {
      sources.push('Admin');
  }
...
function openResultModal(data) {
  ...
  const resultHtml = '<div class="verification-result">'
    ...
    +'<div class="panels-row">'
      ...
      +'<div class="panel metrics"><div class="panel-title"><span class="label">Verification Summary</span></div>'
        +'<ul>'
          +'<li><strong>FB Page:</strong> '+safeText(data.pageName || data.sourceName || getSourceName(data.url) || '')+'</li>'
          +'<li><strong>Fact-Check Sources:</strong> '+(data.sources && Number(data.sources) > 0 ? Number(data.sources) : 'None provided.')+'</li>'
          + getFactCheckedByHtml(data)
          +'<li><strong>Times Verified:</strong> '+Number(data.verificationCount || 1)+'</li>'
          +(data.url ? '<li><strong>URL:</strong> <div style="display:inline-block; max-width:100%; vertical-align:top;"><a href="'+safeText(data.url)+'" target="_blank" rel="noopener" style="display:inline-block; padding:2px 8px; background:rgba(255,255,255,0.05); border-radius:4px; font-size:0.85em; color:#cbd5e1; text-decoration:none; word-break:break-all;">'+safeText(data.url)+'</a></div></li>' : '')
        +'</ul>'
      +'</div>'
    +'</div>'
...
function detectCategory(item){
  ...
  const disaster = ['earthquake','lindol','quake','magnitude','aftershock','tremor','tsunami','flood','landslide','hurricane','cyclone','sunog','volcano','bagyo','bagyong','storm','typhoon'];
  function hits(words){
    let h=0; for(const w of words){ if(text.includes(w)||page.includes(w)||url.includes(w)) h++; } return h;
  }
  const hp = hits(politics), hs = hits(sports), hm = hits(music), hd = hits(disaster);
  const max = Math.max(hp,hs,hm,hd);
  if (max===0) return 'Others';
  if (max===hp) return 'Politics';
  if (max===hs) return 'Sports';
  if (max===hm) return 'Music';
  return 'Disaster';
}

if (categorySelect) {
  categorySelect.addEventListener('change', function(){
    currentCategory = String(this.value || 'all').toLowerCase();
    const uid = getUserId();
    applyFiltersAndRender(uid);
  });
}
