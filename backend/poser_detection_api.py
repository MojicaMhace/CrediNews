...
def compute_poser_score(meta: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # "IMPERSONATION TRAP" PRE-CALCULATION
...
def poser_analyze_full():
    try:
        # [UPDATED] Authentication check removed as per user request (enforcement via Firestore rules)
        # try:
        #     auth_header = request.headers.get("Authorization") or ""
        #     token = auth_header.split("Bearer ")[1] if "Bearer " in auth_header else None
        #     decoded = firebase_auth.verify_id_token(token) if token else None
        ...
        #             data = doc.to_dict() or {}
        #             if not data.get("facebookProviderLinked", False):
        #                 return jsonify({"error": "Forbidden: Facebook not linked"}), 403
        #         except Exception:
        ...
        if db:
            try:
                ...
                if doc.exists:
                    ...
                    # --- FIX: IMPROVED REGISTRY CHECK USING CACHED DATA ---
                    if not reg2:
                        # Try using the canonical link stored in cache (which might have correct casing)
                        cached_link = existing_data.get("link")
                        if cached_link and cached_link != url:
                            reg2 = _check_verified_registry(cached_link)
                        
                        # Try using the username stored in cache
                        if not reg2 and existing_data.get("username"):
                            uname = existing_data.get("username")
                            # Try common variants
                            reg2 = _check_verified_registry(f"https://www.facebook.com/{uname}")
                    # --- END FIX ---

                    is_registry_verified = reg2 is not None
        ...
        print("CRITICAL ERROR in poser_analyze_full:")
        import traceback
        traceback.print_exc()
...
def _check_website_reciprocity(website_url: Optional[str], fb_link: Optional[str], username: Optional[str]) -> Dict[str, bool]:
    ...
    try:
        ...
        if "facebook.com" in html or "fb.com" in html:
            flags["website_has_facebook"] = True
            
            uname = (username or "").strip().lower()
            # Check by Username
            if uname:
                 if (f"facebook.com/{uname}" in html) or (f"fb.com/{uname}" in html):
            ...
            if link:
                # Normalize link for search (remove protocol to match partials)
                simple_link = link.replace("https://", "").replace("http://", "").replace("www.", "").strip("/")
                if simple_link and simple_link in html:
                    flags["website_links_to_page"] = True

    except Exception as e:
        print(f"Reciprocity Check Error: {e}")
        pass
    return flags
